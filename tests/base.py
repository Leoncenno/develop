...
app.config.from_object('pytest_testconfig')
...
class TestUser(unittest.TestCase):
    ...
    def empty_string_update_question(self, empty_string_update_question):
        response = self.client.post('/api/v1/questions/1',headers={'Authorization': 'Bearer '+ self.user_token(login)}\
    ...
    def get_all_questions(self):
        response = self.client.get('/api/v1/questions',headers={'Authorization': 'Bearer '+ self.user_token(login)}\
    ...
    def get_one_question(self):
        response = self.client.get('/api/v1/questions/1',headers={'Authorization': 'Bearer '+ self.user_token(login)}\
        ,content_type = 'application/json')
        return response

    def post_an_answer(self, post_answer):
        response = self.client.post('/api/v1/questions/1/answers', headers={'Authorization': 'Bearer '+ self.user_token(login)}\
    ...
    def get_answers_for_a_question(self):
        response = self.client.get('/api/v1/questions/<id>/answers',headers={'Authorization': 'Bearer '+ self.user_token(login)}\
        ,content_type = 'application/json')
        return response
