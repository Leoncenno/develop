...
from tests.__init__ import (questions, get_all_questions, get_answers_for_one_question, get_one_question, post_answer, post_question,\
                ...
                wrong_input_update_question, sign_up, login_data, missing_data_login, missing_data_sign_up)
...
class TestUser(unittest.TestCase):
    ...
    def signin_user(self, login_data):
    ...
    def post_a_question(self, post_question):
        response = self.client.post('/api/v1/questions', headers={'Authorization': 'Bearer '+ self.user_token(login_data)}\
    ...
    def empty_string_update_question(self, empty_string_update_question):
        response = self.client.post('/api/v1/questions/1',headers={'Authorization': 'Bearer '+ self.user_token(login_data)}\
    ...
    def get_all_questions(self):
        response = self.client.get('/api/v1/questions',headers={'Authorization': 'Bearer '+ self.user_token(login_data)}\
    ...
    def get_one_question(self):
        response = self.client.get('/api/v1/questions/1',headers={'Authorization': 'Bearer '+ self.user_token(login_data)}\
        ,content_type = 'application/json')
        return response

    def post_an_answer(self, post_answer):
        response = self.client.post('/api/v1/questions/1/answers', headers={'Authorization': 'Bearer '+ self.user_token(login_data)}\
    ...
    def get_answers_for_a_question(self):
        response = self.client.get('/api/v1/questions/<id>/answers',headers={'Authorization': 'Bearer '+ self.user_token(login_data)}\
        ,content_type = 'application/json')
        return response
