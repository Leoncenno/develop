...
class TestUser(unittest.TestCase):
    ...
    def signup_user(self, sign_up):
        response = self.client.post(
            '/register', data=json.dumps(sign_up), content_type='application/json')
        return response
    ...
    def signin_user(self, login):
        response = self.client.post(
            '/login', data=json.dumps(login), content_type='application/json')
        return response
    ...
    def post_a_question(self, post_question):
        response = self.client.post('/api/v1/questions', headers={'Authorization': 'Bearer ' + self.user_token(
            login)}, data=json.dumps(post_question), content_type='application/json')
        return response

    def empty_string_question(self, empty_string_question_post):
        response = self.client.post('/api/v1/questions', headers={'Authorization': 'Bearer ' + self.user_token(
            login)}, data=json.dumps(empty_string_question_post), content_type='application/json')
        return response

    def wrong_question_input(self, wrong_input_question):
        response = self.client.post('/api/v1/questions', headers={'Authorization': 'Bearer ' + self.user_token(
            login)}, data=json.dumps(wrong_input_question), content_type='application/json')
        return response

    def empty_string_description(self, empty_string_description_post):
        response = self.client.post('/api/v1/questions', headers={'Authorization': 'Bearer ' + self.user_token(
            login)}, data=json.dumps(empty_string_description_post), content_type='application/json')
        return response

    def wrong_description_input(self, wrong_input_description):
        response = self.client.post('/api/v1/questions', headers={'Authorization': 'Bearer ' + self.user_token(
            login)}, data=json.dumps(wrong_input_description), content_type='application/json')
        return response

    def empty_string_update_question(self, empty_string_update_question):
        response = self.client.post('/api/v1/questions/1', headers={'Authorization': 'Bearer ' + self.user_token(
            login)}, data=json.dumps(empty_string_update_question), content_type='application/json')
        return response

    def wrong_question_update_input(self, wrong_input_update_question):
        response = self.client.post('/api/v1/questions/1', headers={'Authorization': 'Bearer ' + self.user_token(
            login)}, data=json.dumps(wrong_input_update_question), content_type='application/json')
        return response

    def get_all_questions(self):
        response = self.client.get('/api/v1/questions', headers={
                                   'Authorization': 'Bearer ' + self.user_token(login)}, content_type='application/json')
        return response
    ...
    def get_one_question(self):
        response = self.client.get('/api/v1/questions/1', headers={
                                   'Authorization': 'Bearer ' + self.user_token(login)}, content_type='application/json')
        return response

    def post_an_answer(self, post_answer):
        response = self.client.post('/api/v1/questions/1/answers', headers={'Authorization': 'Bearer ' + self.user_token(
            login)}, data=json.dumps(post_answer), content_type='application/json')
        return response

    def empty_string_answer(self, empty_string_answer_post):
        response = self.client.post('/api/v1/questions/1/answers', headers={'Authorization': 'Bearer ' + self.user_token(
            login)}, data=json.dumps(empty_string_answer_post), content_type='application/json')
        return response

    def wrong_answer_input(self, wrong_input_answer):
        response = self.client.post('/api/v1/questions/1/answers', headers={'Authorization': 'Bearer ' + self.user_token(
            login)}, data=json.dumps(wrong_input_answer), content_type='application/json')
        return response

    def get_answers_for_a_question(self):
        response = self.client.get('/api/v1/questions/<id>/answers', headers={
                                   'Authorization': 'Bearer ' + self.user_token(login)}, content_type='application/json')
        return response
